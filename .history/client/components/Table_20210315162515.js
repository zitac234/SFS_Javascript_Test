...
export default class Table extends React.Component{
      ...
      addRow(){
            ...
            console.log(addedRowCheckbox[0])
            for(let i = 0; i < cellBalance.length; i++){
                  cellBalance[i].addEventListener('change', (event)=>{
                        addedRowCheckbox[i].setAttribute('name', `row${index+1}`)
                        addedRowCheckbox[i].setAttribute('value', )
                        // addedRowCheckbox[i].name = `row${index+1}`
                        // addedRowCheckbox[i].value = event.target.value
            ...
            return (this.state.data.map((user, index) =>{
                 return  <tr key={index}>{keys.map((key, num)=>{
                       ...
                       if(num === 0){
                        ...
                        return <td key={num} ><input type="checkbox" className="checkbox" name = {Inputname }  value={rowBalance}></input></td> 
                       }else{
